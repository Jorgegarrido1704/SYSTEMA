...
class InventariosController extends Controller
{
    ...
    public function index_inventarios()
    {
        ...
        if ($cat == "invreg1" || $cat == "invreg2" || $cat == "capt" || $cat == "invwo1" || $cat == "invwo2") {
            ...
            $datosRegistros = globalInventarios::where('difference', '>', 0)->orderBy('difference', 'desc')->get();
        }
    ...
    public function addInventarios(Request $request)
    {
        ...
        if ($cat == "invreg1") {
            $buscarfolios = globalInventarios::where('Folio_sheet_audited', '=', $request->input('folios'),'AND','items','=',$request->input('item'))->first();
            if ($buscarfolios) {
                $primero=$buscarfolios->first_qty_count;
                $actualizado=globalInventarios::where('Folio_sheet_audited', '=', $request->input('folios'),'AND','items','=',$request->input('item'))->update([
            ...
            if (!$buscarfolios) {
                ...
                if($buscarfolios->second_qty_count != null or $buscarfolios->second_qty_count != 0){
                    $sum=$buscarfolios->second_qty_count+$request->input('qty');
                    $difference = abs($buscarfolios->first_qty_count - $sum);
                    $agregar= globalInventarios::where('Folio_sheet_audited', '=', $request->input('folios'))->where('items','=',$request->input('item'))->update([
                        "second_qty_count"=>$sum,
