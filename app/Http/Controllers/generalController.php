...
class generalController extends Controller
{
    ...
    public function datosOrdenesGeneral(Request $request)
    {
        try {
            ...
            foreach ($buscar as $row) {
                ...
                $tableContent .= '<td>'.$row->planpar.'</td>';
                $tableContent .= '<td>'.$row->precut.'</td>';
                $tableContent .= '<td>'.$row->tobecut.'</td>';
                $tableContent .= '<td>'.$row->cortPar.'</td>';
                $tableContent .= '<td>'.$row->preterm.'</td>';
                $tableContent .= '<td>'.$row->tobeterm.'</td>';
                $tableContent .= '<td>'.$row->libePar.'</td>';
                $tableContent .= '<td>'.$row->preassembly.'</td>';
                $tableContent .= '<td>'.$row->tobeassembly.'</td>';
                $tableContent .= '<td>'.$row->ensaPar.'</td>';
                $tableContent .= '<td>'.$row->preloom.'</td>';
                $tableContent .= '<td>'.$row->tobeloom.'</td>';
    ...
    public function registrar_work(request $request, $wo)
    {
        ...
        $ensa = $cuentas->ensaPar + $cuentas->preassembly + $cuentas->tobeassembly + $cuentas->specialWire;
        $loom = $cuentas->loomPar + $cuentas->preloom + $cuentas->tobeloom;
        $calidad = $cuentas->testPar + $cuentas->fallasCalidad + $cuentas->preCalidad;
        $eng = $cuentas->eng;
        $donde = '';
        $count = 6;
